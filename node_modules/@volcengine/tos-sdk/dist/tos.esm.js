...
const isValidVectorBucketName = bucket => {
  if (bucket) {
    if (bucket.length < 3 || bucket.length > 32) {
      throw new TosClientError('invalid vector bucket name, the length must be [3, 32]');
    }

    if (!/^([a-z]|-|\d)+$/.test(bucket)) {
      throw new TosClientError('invalid vector bucket name, the character set is illegal');
    }

    if (/^-/.test(bucket) || /-$/.test(bucket)) {
      throw new TosClientError(`invalid vector bucket name, the vector bucket name can be neither starting with '-' nor ending with '-'`);
    }
  }
};
...
(function (StorageClassType) {
  ...
  StorageClassType["StorageClassDeepColdArchive"] = "DEEP_COLD_ARCHIVE";
...
(function (AuditType) {
  AuditType["Image"] = "image";
  AuditType["Video"] = "video";
  AuditType["Audio"] = "audio";
})(AuditType || (AuditType = {}));
/**
 * @private unstable
 */
...
var CallbackTriggerModeType;

(function (CallbackTriggerModeType) {
  CallbackTriggerModeType["Freeze"] = "freeze";
  CallbackTriggerModeType["Sensitive"] = "sensitive";
  CallbackTriggerModeType["Failure"] = "failure";
})(CallbackTriggerModeType || (CallbackTriggerModeType = {}));
/**
 * @private unstable
 */
...
(function (FreezeModeType) {
  FreezeModeType["Archive"] = "archive";
  FreezeModeType["Private"] = "private";
})(FreezeModeType || (FreezeModeType = {}));
/**
 * @private unstable
 */
...
(function (TemplateTagType) {
  TemplateTagType["AudioConvert"] = "AudioConvert";
  TemplateTagType["Transcode"] = "Transcode";
})(TemplateTagType || (TemplateTagType = {}));
/**
 * @private unstable
 */


var AudioContainerFormatType;

(function (AudioContainerFormatType) {
  AudioContainerFormatType["Wav"] = "wav";
  AudioContainerFormatType["Mp3"] = "mp3";
  AudioContainerFormatType["Aac"] = "aac";
  AudioContainerFormatType["Flac"] = "flac";
  AudioContainerFormatType["Oga"] = "oga";
  AudioContainerFormatType["Ac3"] = "ac3";
  AudioContainerFormatType["Opus"] = "opus";
})(AudioContainerFormatType || (AudioContainerFormatType = {}));
/**
 * @private unstable
 */


var ContainerFormatType;

(function (ContainerFormatType) {
  ContainerFormatType["Mp4"] = "mp4";
  ContainerFormatType["Ts"] = "ts";
...
var SampleFormatType;

(function (SampleFormatType) {
  SampleFormatType["Fltp"] = "fltp";
  SampleFormatType["S16p"] = "s16p";
  SampleFormatType["S32p"] = "s32p";
  SampleFormatType["S16"] = "s16";
  SampleFormatType["S32"] = "s32";
})(SampleFormatType || (SampleFormatType = {}));

var DataType;

(function (DataType) {
  DataType["DataTypeFloat32"] = "float32";
})(DataType || (DataType = {}));

var DistanceMetricType;

(function (DistanceMetricType) {
  DistanceMetricType["DistanceMetricEuclidean"] = "euclidean";
  DistanceMetricType["DistanceMetricCosine"] = "cosine";
  ...
  const nextEnsureCloseFd = async () => {
    ...
    if (checkpointRichInfo.record) {
      ...
      try {
        ...
        if ('node' === 'node') {
          // 3. [仅 Windows] 设置文件为稀疏文件
          if (process.platform === 'win32' && enableSparseFile && objectSize > partSize) {
            const {
              execFile
            } = require('child_process');

            await new Promise(resolve => {
              execFile('fsutil', ['sparse', 'setflag', tempFilePath], (err, _stdout, stderr) => {
                if (err) {
                  console.warn('设置稀疏文件标志失败:', tempFilePath, err, stderr); // 非致命错误，继续执行
                } else {
                  console.warn('设置稀疏文件标志成功:', tempFilePath);
                }
...
class TOSBase {
  ...
  normalizeOpts(_opts) {
    ...
    const S3_REGION_LIST = ['tos-s3-cn-beijing.volces.com', 'tos-s3-cn-guangzhou.volces.com', 'tos-s3-cn-shanghai.volces.com', 'tos-s3-ap-southeast-1.volces.com'];
  ...
  async fetch(method, path, query, headers, body, opts) {
    ...
    const sig = new SignersV4({
      ...
      serviceName: (opts == null ? void 0 : opts.serviceName) || 'tos',
      bucket: '',
    ...
    try {
      ...
      var _err$response, _err$response$headers, _err$response2, _err$response2$data;

      if (axios.isAxiosError(err) && ((_err$response = err.response) != null && (_err$response$headers = _err$response.headers) != null && _err$response$headers['x-tos-request-id'] || (_err$response2 = err.response) != null && (_err$response2$data = _err$response2.data) != null && _err$response2$data.requestId)) {
...
async function createBucket(input) {
  ...
  fillRequestHeaders(input, ['acl', 'grantFullControl', 'grantRead', 'grantReadAcp', 'grantWrite', 'grantWriteAcp', 'storageClass', 'azRedundancy', 'bucketType', 'tagging']);
  /**
   * empty string is invalid value
   */
...
async function doesBucketExist(input) {
  if (!input) {
...
async function getBucketInfo(input) {
  const {
    bucket
  } = input;
...
async function getFetchTask(input) {
...
async function doesObjectExist(input) {
  if (!input) {
    throw new TosClientError('input is required');
  }
...
(function (InventoryOptionalFields) {
  ...
  InventoryOptionalFields["IntelligentTieringAccessTier"] = "IntelligentTieringAccessTier";
...
async function getAcceleratorCapacityQuota(input) {
  const {
    AccountId,
    Az
  } = input;
  const res = await this.fetch('GET', '/accelerator/capacity', {
    az: Az
  }, {
    'x-tos-account-id': AccountId
  }, {}, {});
...
async function listBindAcceleratorForAccessPoint(input) {
  const {
    AccountId,
    AccessPointName
  } = input;
  const res = await this.fetch('GET', `/accesspoint/${AccessPointName}/accelerator`, {}, {
    'x-tos-account-id': AccountId
  }, {}, {});
  return res;
}
...
async function postBucketIncrementAudit(input) {
  const {
    bucket,
    job
  } = input;
...
async function listBucketIncrementAudits(input) {
  const {
    bucket,
    pageSize,
    pageToken,
...
async function getBucketIncrementAudit(input) {
  const {
    bucket,
    ruleId
  } = input;
...
async function deleteBucketIncrementAudit(input) {
  const {
    bucket,
    ruleId
  } = input;
...
var DatasetTemplateId;

(function (DatasetTemplateId) {
  DatasetTemplateId["BasicInfoTemplate"] = "basic_info_template";
  DatasetTemplateId["ImageSemanticTemplate"] = "image_semantic_template";
})(DatasetTemplateId || (DatasetTemplateId = {}));
/**
 * 创建Dataset接口
 * @param this TOSBase实例
 * @param input CreateDatasetParams
 * @returns CreateDatasetOutput
 * @private unstable 暂不对外
 */
...
async function createDataset(input) {
  ...
  const res = await this.fetch('POST', '/dataset', {}, {
    'x-tos-account-id': accountId
...
async function getDataset(input) {
  const {
    datasetname,
    accountId
  } = input;
  const res = await this.fetch('GET', '/dataset', {
    datasetname
  }, {
    'x-tos-account-id': accountId
  });
...
async function deleteDataset(input) {
  const {
    accountId,
    datasetname
  } = input;
  const res = await this.fetch('DELETE', '/dataset', {
    datasetname
  }, {
    'x-tos-account-id': accountId
  });
...
async function listDataset(input) {
  const {
    accountId,
    ...rest
  } = input;
  const res = await this.fetch('GET', '/datasets', filterUndefined(rest), {
    'x-tos-account-id': accountId
  });
  return res;
}
...
(function (STOCK_STATE) {
  STOCK_STATE["INDEXING"] = "Indexing";
  STOCK_STATE["DONE"] = "Done";
  STOCK_STATE["FAILED"] = "Failed";
...
async function createDatasetBinding(input) {
  const {
    accountId,
    ...rest
  } = input;
  const res = await this.fetch('POST', '/datasetbinding', {}, {
    'x-tos-account-id': accountId
  }, filterUndefined(rest));
  return res;
}
...
async function deleteDatasetBinding(input) {
  const {
    accountId,
    ...rest
  } = input;
  const res = await this.fetch('DELETE', '/datasetbinding', {}, {
    'x-tos-account-id': accountId
  }, filterUndefined(rest), {});
  return res;
}
...
async function getDatasetBinding(input) {
  const {
    accountId,
    ...rest
  } = input;
  const res = await this.fetch('GET', '/datasetbinding', filterUndefined(rest), {
    'x-tos-account-id': accountId
  });
  return res;
}
...
async function listDatasetBindings(input) {
  const {
    accountId,
    ...rest
  } = input;
  const res = await this.fetch('GET', '/datasetbindings', filterUndefined(rest), {
    'x-tos-account-id': accountId
  });
  return res;
}
...
(function (Operator) {
  ...
  Operator["MATCH_PHRASE"] = "match-phrase";
  /** 嵌套查询 - 字段为数组时，同一对象内的逻辑条件查询 */
  // NESTED = 'nested',
})(Operator || (Operator = {}));

var AGGREGATION_OPERATION;

(function (AGGREGATION_OPERATION) {
  AGGREGATION_OPERATION["MIN"] = "min";
  AGGREGATION_OPERATION["MAX"] = "max";
  AGGREGATION_OPERATION["AVERAGE"] = "average";
  AGGREGATION_OPERATION["SUM"] = "sum";
  AGGREGATION_OPERATION["COUNT"] = "count";
  AGGREGATION_OPERATION["DISTINCT"] = "distinct";
  AGGREGATION_OPERATION["GROUP"] = "group";
})(AGGREGATION_OPERATION || (AGGREGATION_OPERATION = {}));

async function datasetSimpleQuery(input) {
  const {
    accountId,
    ...rest
  } = input;
  const res = await this.fetch('POST', '/datasetquery', {
    mode: 'SimpleQuery'
  }, {
    'x-tos-account-id': accountId
  }, filterUndefined(rest));
...
var SEMANTIC_QUERY_TYPE;

(function (SEMANTIC_QUERY_TYPE) {
  SEMANTIC_QUERY_TYPE["TEXT"] = "text";
  SEMANTIC_QUERY_TYPE["IMAGE"] = "image";
})(SEMANTIC_QUERY_TYPE || (SEMANTIC_QUERY_TYPE = {}));

async function datasetSemanticQuery(input) {
  const {
    accountId,
    ...rest
  } = input;
...
async function putBucketDataProcessTemplate(input) {
  const {
    bucket,
    tag,
    name,
...
async function listBucketDataProcessTemplates(input) {
  const {
    bucket,
    tag,
    prefix
...
async function getBucketDataProcessTemplate(input) {
  const {
    bucket,
    id,
    tag
...
async function deleteBucketDataProcessTemplate(input) {
  const {
    bucket,
    id
  } = input;
 ...
 * @private unstable method
 */
async function putAudioConvertTemplate(input) {
  const {
    bucket,
    name,
    audioConvertConfig
  } = input; // Build query parameters
  ...
  const res = await this.fetchBucket(bucket, 'PUT', query, headers, body); // Ensure res.data exists and contains templateId
...
async function getAudioConvertTemplate(input) {
  const {
    bucket,
    id
  } = input; // Build query parameters
...
async function listAudioConvertTemplates(input) {
  const {
    bucket,
    id
  } = input; // Build query parameters
  ...
  const res = await this.fetchBucket(bucket, 'GET', query, {});
  res.data = {
    AudioConvertTemplates: res.data || []
  };
  return res;
}
/**
...
async function deleteAudioConvertTemplate(input) {
  const {
    bucket,
    id
  } = input; // Build query parameters
...
async function putVideoConvertTemplate(input) {
  const {
    bucket,
    name,
    transcodeConfig
  ...
  const headers = {}; // Build request body
  ...
  const res = await this.fetchBucket(bucket, 'PUT', query, headers, body);
  return { ...res,
    data: {
      ID: res.data
    }
...
async function getVideoConvertTemplate(input) {
  const {
    bucket,
    id
  } = input; // Build query parameters
...
async function deleteVideoConvertTemplate(input) {
  const {
    bucket,
    id
  } = input; // Build query parameters
...
async function listVideoConvertTemplates(input) {
  const {
    bucket
  } = input; // Build query parameters
  ...
  const res = await this.fetchBucket(bucket, 'GET', query, {});
  res.data = {
    VideoConvertTemplates: res.data || []
  };
  return res;
}

async function createVideoConvertJob(input) {
  const {
    bucket,
    input: jobInput,
    transcodeConfig,
  ...
  const headers = {}; // 构建请求体 - 由于接口字段已经是大驼峰格式，可以直接使用
  ...
  const body = {
    ...
    TranscodeConfig: transcodeConfig,
    Output: output
  }; // 发送请求 - 使用 fetchBucket 因为这是存储桶级别的操作

  const res = await this.fetchBucket(bucket, 'POST', query, headers, body);
  return res;
}
async function getVideoConvertJob(input) {
  const {
    bucket,
    jobId
  } = input; // 手动构建查询参数
  ...
  const headers = {}; // 发送请求 - 使用 fetchBucket 因为这是存储桶级别的操作

  const res = await this.fetchBucket(bucket, 'GET', query, headers);
  return res;
}
/**
 * @private unstable method
...
async function createAudioConvertJob(input) {
  const {
    bucket,
    input: jobInput,
    audioConvertConfig,
...
async function getAudioConvertJob(input) {
  const {
    bucket,
    jobId
  } = input; // 手动构建查询参数
  ...
  const res = await this.fetchBucket(bucket, 'GET', query, headers); // 直接返回响应
...
async function listTemplates(input) {
  ...
  const res = await this.fetch('GET', '/templates', {}, // 无查询参数
  {
    'x-tos-account-id': accountId
  }, {});
  return res;
...
const CommonQueryKey$c = 'redundancyTransition';
/**
 * @private unstable method
 * 创建桶数据冗余转换任务
 * @param this
...
async function createBucketDataRedundancyTransition(input) {
  const {
    bucket,
    targetRedundancyType
  } = input;
  return this.fetchBucket(bucket, 'POST', {
    [CommonQueryKey$c]: '',
...
/**
 * @private unstable method
 * 根据 taskId 获取桶数据冗余转换任务
 * @param this
 * @param input
 * @returns
 */

async function getBucketDataRedundancyTransition(input) {
  const {
    bucket,
    redundancyTransitionTaskId
  } = input;
  return await this.fetchBucket(bucket, 'GET', {
    [CommonQueryKey$c]: '',
...
/**
 * @private unstable method
 * 根据 taskId 删除桶数据冗余转换任务
 * @param this
 * @param input
 * @returns
 */

async function deleteBucketDataRedundancyTransition(input) {
  const {
    bucket,
    redundancyTransitionTaskId
  } = input;
...
async function listBucketDataRedundancyTransition(input) {
  const {
    bucket,
    maxKeys,
    continuationToken
...
async function getBucketTransitionEstimatedRemainingTime(input) {
  const {
    bucket
  } = input;
  return this.fetchBucket(bucket, 'GET', {
...
async function listUserDataRedundancyTransition(input) {
  const {
    continuationToken,
    maxKeys
  } = input;
 ...
 * @private unstable method
 */
async function putConvertWorkflow(input) {
  const {
    bucket,
    rules
  } = input;
...
async function getConvertWorkflow(input) {
  const {
    bucket
  } = input;
  return this.fetchBucket(bucket, 'GET', {
...
async function deleteConvertWorkflow(input) {
  const {
    bucket
  } = input;
  const query = {
...
async function putAnyPrivateAccessPoint(input) {
  const {
    Name,
    AccountId,
    VpcId,
  ...
  const res = await this.fetch('PUT', `/apap`, {
    name: Name
  }, {
    'x-tos-account-id': AccountId
  }, filterUndefined({
...
async function listAnyPrivateAccessPoints(input) {
  const {
    AccountId,
    ...nextQuery
  } = input;
  const res = await this.fetch('GET', '/apap', filterUndefined(nextQuery), {
    'x-tos-account-id': AccountId
  });
  return res;
}
...
async function listAnyPrivateAccessPointSupportAz(input) {
  const {
    AccountId
  } = input;
  const res = await this.fetch('GET', '/apap/az', {}, {
    'x-tos-account-id': AccountId
  });
  return res;
}
...
async function listAnyPrivateAccessPointEndpoints(input) {
  const {
    AccountId
  } = input;
  const res = await this.fetch('GET', '/apap/endpoint', {}, {
    'x-tos-account-id': AccountId
  });
  return res;
}
...
class InnerClient extends TOSBase {
  constructor(...args) {
    ...
    this.getFetchTask = getFetchTask;
    this.listObjectsType2 = listObjectsType2;
    this.listObjectVersions = listObjectVersions;
    ...
    this.listBindAcceleratorForAccessPoint = listBindAcceleratorForAccessPoint;
    ...
    this.getAcceleratorCapacityQuota = getAcceleratorCapacityQuota;
    ...
    this.putConvertWorkflow = putConvertWorkflow;
    this.getConvertWorkflow = getConvertWorkflow;
    this.deleteConvertWorkflow = deleteConvertWorkflow;
    ...
    this.postBucketIncrementAudit = postBucketIncrementAudit;
    this.listBucketIncrementAudits = listBucketIncrementAudits;
    this.getBucketIncrementAudit = getBucketIncrementAudit;
    this.deleteBucketIncrementAudit = deleteBucketIncrementAudit;
    ...
    this.getAudioConvertJob = getAudioConvertJob;
    this.createAudioConvertJob = createAudioConvertJob;
    ...
    this.listDatasetBindings = listDatasetBindings;
    this.getDatasetBinding = getDatasetBinding;
    this.datasetSimpleQuery = datasetSimpleQuery;
    this.listTemplates = listTemplates;
    this.datasetSemanticQuery = datasetSemanticQuery;
    this.putBucketDataProcessTemplate = putBucketDataProcessTemplate;
    this.putAudioConvertTemplate = putAudioConvertTemplate;
    this.getBucketDataProcessTemplate = getBucketDataProcessTemplate;
    this.deleteBucketDataProcessTemplate = deleteBucketDataProcessTemplate;
    this.listBucketDataProcessTemplates = listBucketDataProcessTemplates;
    this.getAudioConvertTemplate = getAudioConvertTemplate;
    this.deleteAudioConvertTemplate = deleteAudioConvertTemplate;
    this.listAudioConvertTemplates = listAudioConvertTemplates;
    this.putVideoConvertTemplate = putVideoConvertTemplate;
    this.getVideoConvertTemplate = getVideoConvertTemplate;
    this.deleteVideoConvertTemplate = deleteVideoConvertTemplate;
    this.listVideoConvertTemplates = listVideoConvertTemplates;
    this.createVideoConvertJob = createVideoConvertJob;
    this.getVideoConvertJob = getVideoConvertJob;
    this.createBucketDataRedundancyTransition = createBucketDataRedundancyTransition;
    this.getBucketDataRedundancyTransition = getBucketDataRedundancyTransition;
    this.listBucketDataRedundancyTransition = listBucketDataRedundancyTransition;
    this.deleteBucketDataRedundancyTransition = deleteBucketDataRedundancyTransition;
    this.listUserDataRedundancyTransition = listUserDataRedundancyTransition;
    this.getBucketTransitionEstimatedRemainingTime = getBucketTransitionEstimatedRemainingTime;
    this.putAnyPrivateAccessPoint = putAnyPrivateAccessPoint;
    this.listAnyPrivateAccessPoints = listAnyPrivateAccessPoints;
    this.listAnyPrivateAccessPointSupportAz = listAnyPrivateAccessPointSupportAz;
    this.listAnyPrivateAccessPointEndpoints = listAnyPrivateAccessPointEndpoints;
  }

}

async function createVectorBucket(input) {
  const {
    vectorBucketName
  } = input;
  const res = await this.fetchVectors('POST', '/CreateVectorBucket', {}, {}, {
    vectorBucketName
  });
  res.data = {};
  return res;
}
async function listVectorBuckets(input) {
  const {
    prefix,
    nextToken,
    maxResults,
  ...
  return this.fetchVectors('POST', '/ListVectorBuckets', {}, headers, {
    prefix,
    nextToken,
    maxResults
  });
}
async function getVectorBucket(input) {
  const {
    accountID
  } = input;
  const query = {};
  ...
  const res = await this.fetchVectors('POST', '/GetVectorBucket', query, header, body, {
    bucket: input.vectorBucketName,
    accountId: accountID
  });
  return res;
}
async function deleteVectorBucket(input) {
  const {
    vectorBucketName,
    accountID
  } = input;
  ...
  const res = await this.fetchVectors('POST', '/DeleteVectorBucket', query, header, body, {
...
async function putVectorBucketPolicy(input) {
  const headers = {};
  const body = {
    vectorBucketName: input.vectorBucketName,
    policy: input.policy
  };
  const res = await this.fetchVectors('POST', '/PutVectorBucketPolicy', {}, headers, body, {
    bucket: input.vectorBucketName,
...
async function getVectorBucketPolicy(input) {
  const {
    vectorBucketName,
    accountId
  } = input;
  ...
  const res = await this.fetchVectors('POST', '/GetVectorBucketPolicy', {}, headers, body, {
    accountId,
    bucket: vectorBucketName
  });
  return res;
}
async function deleteVectorBucketPolicy(input) {
  const headers = {};
  const body = {
    vectorBucketName: input.vectorBucketName
  };
  const res = await this.fetchVectors('POST', '/DeleteVectorBucketPolicy', {}, headers, body, {
    bucket: input.vectorBucketName,
    accountId: input.accountID
  });
  res.data = {};
...
async function createIndex(input) {
  const {
    vectorBucketName,
    accountID
  } = input; // 根据 input 构造 query

  const query = {}; // 根据 input 构造 header

  const header = {}; // 根据 input 构造 body
  ...
  const res = await this.fetchVectors('POST', '/CreateIndex', query, header, body, {
    bucket: vectorBucketName,
    accountId: accountID
  });
  return res;
}
async function deleteIndex(input) {
  const {
    vectorBucketName,
    accountID
  } = input; // 根据 input 构造 query
  ...
  const res = await this.fetchVectors('POST', '/DeleteIndex', query, header, body, {
    bucket: vectorBucketName,
    accountId: accountID
  });
  res.data = {};
...
async function listIndexes(input) {
  const {
    vectorBucketName,
    accountID
  } = input; // 根据 input 构造 query
  ...
  const res = await this.fetchVectors('POST', '/ListIndexes', query, header, body, {
    bucket: vectorBucketName,
    accountId: accountID
  });
  return res;
...
async function getIndex(input) {
  ...
  const res = await this.fetchVectors('POST', '/GetIndex', {}, headers, body, {
    accountId: input.accountID,
    bucket: input.vectorBucketName
  });
  return res;
...
async function putVectors(input) {
  const {
    vectorBucketName,
    accountID
  } = input; // 根据 input 构造 query
  ...
  const res = await this.fetchVectors('POST', '/PutVectors', query, header, body, {
    bucket: vectorBucketName,
    accountId: accountID
  });
  return res;
}
async function listVectors(input) {
  const {
    vectorBucketName,
    accountID
  } = input; // 根据 input 构造 query
  ...
  const res = await this.fetchVectors('POST', '/ListVectors', query, header, body, {
    bucket: vectorBucketName,
    accountId: accountID
  });
  return res;
}
async function getVectors(input) {
  const {
    vectorBucketName,
    accountID
  } = input; // 根据 input 构造 query
  ...
  const res = await this.fetchVectors('POST', '/GetVectors', query, header, body, {
    bucket: vectorBucketName,
    accountId: accountID
  });
  return res;
...
async function deleteVectors(input) {
  ...
  const res = await this.fetchVectors('POST', '/DeleteVectors', query, header, body, {
    bucket: vectorBucketName,
    accountId: accountID
  });
  res.data = {};
...
async function queryVectors(input) {
  const {
    vectorBucketName,
    accountID
  } = input;
  ...
  const res = await this.fetchVectors('POST', '/QueryVectors', query, headers, body, {
    bucket: vectorBucketName,
    accountId: accountID
  });
  return res;
...
class TosVectorsClient extends TOSBase {
  constructor(options) {
    super(options);
    this.serviceName = 'tosvectors';
    this.createVectorBucket = createVectorBucket;
    this.listVectorBuckets = listVectorBuckets;
    this.getVectorBucket = getVectorBucket;
    this.deleteVectorBucket = deleteVectorBucket;
    this.putVectorBucketPolicy = putVectorBucketPolicy;
    this.getVectorBucketPolicy = getVectorBucketPolicy;
    this.deleteVectorBucketPolicy = deleteVectorBucketPolicy;
    this.createIndex = createIndex;
    this.deleteIndex = deleteIndex;
    this.getIndex = getIndex;
    this.listIndexes = listIndexes;
    this.putVectors = putVectors;
    this.listVectors = listVectors;
    this.getVectors = getVectors;
    this.deleteVectors = deleteVectors;
    this.queryVectors = queryVectors;
  }

  async fetchVectors(method, path, query, headers, body, opts) {
    if ((opts == null ? void 0 : opts.bucket) !== undefined) {
      isValidVectorBucketName(opts == null ? void 0 : opts.bucket);
    }

    if ((opts == null ? void 0 : opts.accountId) !== undefined) {
      isValidAccountId(opts == null ? void 0 : opts.accountId);
    }

    return this.fetch(method, path, query, headers, body, {
      serviceName: this.serviceName,
      subdomainBucket: opts != null && opts.accountId ? `${opts == null ? void 0 : opts.bucket}-${opts == null ? void 0 : opts.accountId}` : undefined
    });
  }
...
TosClient.AuditType = AuditType;
TosClient.CallbackTriggerModeType = CallbackTriggerModeType;
TosClient.FreezeModeType = FreezeModeType;
TosClient.TemplateTagType = TemplateTagType;
TosClient.AudioContainerFormatType = AudioContainerFormatType;
TosClient.SampleFormatType = SampleFormatType;

export default TosClient;
export { ACLType, AcceleratorPrefetchJobStatus, AccessPointStatus, AccessPointStatusType, AudioContainerFormatType, AuditJobStateType, AuditScanType, AuditType, AzRedundancyType, CallbackTriggerModeType, CancelError, CancelToken, CannedType, DataTransferType, DataType, DistanceMetricType, DownloadEventType, FileCompressFlatten, FileUncompressPrefixReplaced, FreezeModeType, GranteeType, HttpMethodType, LifecycleStatusType, MRAPMirrorBackRedirectPolicyType, MetadataDirectiveType, PermissionType, RedirectType, ReplicationStatusType, ResumableCopyEventType, SampleFormatType, ShareLinkClient, StatusType, StorageClassInheritDirectiveType, StorageClassType, TosClient as TOS, TemplateTagType, TierType, TosClient, TosClientError, TosServerCode, TosServerError, TosVectorsClient, TransferAccelerationStatusType, UploadEventType, VersioningStatusType, WorkflowExecutionStateType, createDefaultRateLimiter$1 as createDefaultRateLimiter, isCancelError as isCancel };
